...
func connect(ctx context.Context, c *Connector, log optionalLogger, p connectParams) (res *tdsSession, err error) {
	...
	if p.instance != "" {
		...
		if err != nil {
			...
			t.Err = fmt.Errorf(f, p.host, err.Error())
		...
		if !ok {
			...
			t.Err = fmt.Errorf(f, p.instance, err.Error())
		...
		if err != nil {
			...
			t.Err = fmt.Errorf(f, strport, err.Error())
...
initiate_connection:
	...
	if encrypt != encryptNotSup {
		...
		// fix for https://github.com/britt/go-mssqldb/issues/166
